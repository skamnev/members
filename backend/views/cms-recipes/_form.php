...
<div class="cms-recipes-form">
    ...
    foreach($languages as $key => $language) {
        $field_title = $form->field($model, "title_$language->url")->textInput(['maxlength' => true])->label('Recipe Title');
        ...
        $field_content_heading = $form->field($model, "content_heading_$language->url")->textInput(['maxlength' => true])->label(Yii::t('backend','Recipe Heading'));
    ...
    <?= $form->field($model, 'category_id')->dropDownList(ArrayHelper::map(\backend\models\CmsRecipesCategories::find()->all(), 'id', 'title'), ['multiple' => 'true', 'size' => 7/*, 'prompt'=>Yii::t('backend', 'Select Categories')*/])->label(Yii::t('backend', 'Select Categories')) ?>

    <?= $form->field($model, 'identifier')->textInput(['maxlength' => true]) ?>

    <?= $form->field($model, 'code_id')->dropDownList(ArrayHelper::map(MappingQuestionsCodes::find()->all(), 'id', 'code'), ['multiple' => 'true', 'size' => 7, 'prompt'=>Yii::t('backend', 'Select Codes')]) ?>

    <?= $form->field($model, 'no_code_id')->dropDownList(ArrayHelper::map(MappingQuestionsCodes::find()->all(), 'id', 'code'), ['multiple' => 'true', 'size' => 7, 'prompt'=>Yii::t('backend', 'Select Codes')]) ?>
